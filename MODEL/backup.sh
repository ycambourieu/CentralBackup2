...
function Archiv(){
	if [[ $ACTIV_CLEANUP == "1" ]];then
		...
		AfficherOk $TEXT
		AppendtoLog $TEXT
		NB_FICHIERS=$(find $DEST_DIR -name "*.tar.gz" -type f | wc -l)
		if [[ $NB_FICHIERS -ge $NB_ARCHIV ]];then
			TEXT="Archivage nécessaire $NB_FICHIERS fichiers de sauvegarde présents"
			AfficherOk $TEXT
			AppendtoLog $TEXT
			AfficherWaiting "Suppression des anciennes sauvegardes"
			AfficherInProgressOverwrite "Suppression des anciennes sauvegardes"
			NB_ARCHIV=$(($NB_ARCHIV-1))
			ls -1rt $(find $DEST_DIR -name "*.tar.gz" -type f) | head -n-${NB_ARCHIV} | xargs rm -f
			AfficherDoneOverwrite "Suppression des anciennes sauvegardes"
		else
			TEXT="Pas d'archivage nécessaire"
			AfficherOk $TEXT
			AppendtoLog $TEXT
		fi
	fi
